...
export function MessageDock({
  ...
  const [collapsedWidth, setCollapsedWidth] = useState<number>(200);

  // Calculate collapsed width based on content
  useEffect(() => {
    const sparkleButton = showSparkleButton ? 48 : 0; // ✨ button width
    const menuButton = showMenuButton ? 48 : 0; // menu button (三个横杠) width
    const dividers = 8; // 2 dividers × 4px each
    const padding = 32; // left and right padding (16px each side)
    const iconGap = 8; // gap between icons
    
    // Count actual assistant icons (excluding sparkle which is handled separately)
    const assistantCount = characters.filter((_, index) => index > 0).length;
    const assistantIcons = assistantCount * 40; // each icon is 40px
    const gaps = assistantCount > 0 ? (assistantCount - 1) * iconGap : 0; // gaps between assistant icons
    
    const calculatedWidth = sparkleButton + assistantIcons + gaps + menuButton + dividers + padding;
    setCollapsedWidth(Math.max(calculatedWidth, 200)); // minimum width of 200px
  }, [characters.length, showSparkleButton, showMenuButton]);
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          "rounded-full px-4 py-2 border shadow-[0px_10px_50px_0px_rgba(0,0,0,0.1)]",
          theme === "dark"
            ? "border-white/10"
            : "border-gray-200/50"
        )}
          ...
          <AnimatePresence>
            {isExpanded && (
              <motion.input
                ...
                className={cn(
                  "flex-1 absolute left-14 right-14 bg-transparent border-none outline-none text-sm font-medium z-50",
                  theme === "dark"
